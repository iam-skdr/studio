    ...
    <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mt-4 flex h-16 items-center justify-between rounded-xl glassmorphism px-6">
          <a href="#" className="flex items-center gap-2 text-2xl font-bold text-primary-foreground hover:text-primary transition-colors">
          ...
          <nav className="hidden md:flex items-center gap-2">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="rounded-md px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-white/10 hover:text-primary-foreground transition-all"
              >
                {link.label}
              </a>
            ))}
          ...
          <nav className="flex flex-col gap-4">
            {navLinks.map((link) => (
              <a
                ...
                className="block rounded-md px-3 py-2 text-base font-medium text-muted-foreground hover:bg-white/10 hover:text-primary-foreground transition-all"
              >
                {link.label}
              </a>
            ))}
